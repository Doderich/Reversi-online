...
export default function WaitingPlayers() {
  ...
  useEffect(() => {
    ...
    socket.on("waiting players", ({ waitingPlayers }: { waitingPlayers: string[] }) => {
  ...
  return (
    <div>
      ...
      <Dialog open={openMatchFoundDialog} onOpenChange={setOpenMatchFoundDialog}>
        <DialogContent>
            <p>Match Found</p>
            <p>Game ID: {game.game_id}</p>
            <Button onClick={() => router.push(`/game/${game.game_id}`)}>Go to Game</Button>
        </DialogContent>
        </Dialog>
      <div>
